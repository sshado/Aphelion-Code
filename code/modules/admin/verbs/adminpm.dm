...
/client/proc/cmd_admin_pm(var/client/C, var/msg = null)
	...
	for(var/client/X in admins)
		...
		if(X.key != key && X.key != C.key && (X.holder.rights & R_ADMIN|R_MOD|R_MENTOR))
			X << "<span class='pm'><span class='other'>" + create_text_tag("pm_other", "PM:", X) + "<A HREF='?_src_=holder;takeadminhelp=\ref[C]'>TAKE</A>" + " <span class='name'>[key_name(src, X, 0)]</span> to <span class='name'>[key_name(C, X, 0)]</span>: <span class='message'>[msg]</span></span></span>"
